...
1. **Parameter Order Confusion**
   ```java
   // Which is correct?
   new ConnectNGame(6, 7, 4);  // rows, columns, winLength?
   new ConnectNGame(7, 6, 4);  // columns, rows, winLength?
...
3. **Difficult to Add Optional Parameters**
   ...
   new ConnectNGame(6, 7, 4, Player.PLAYER_TWO, 'A', 'B', 60, Difficulty.HARD);
...
new ConnectNGameBuilder().rows(6).columns(7).winLength(4).build();
new ConnectNGameBuilder().winLength(4).rows(6).columns(7).build();
new ConnectNGameBuilder().columns(7).winLength(4).rows(6).build();
```
...
ConnectNGame oldStyle = new ConnectNGameBuilder()
    .rows(6)
    .columns(7)
    .winLength(4)
    .build();
...
```java
// Original code
ConnectNGame game1 = new ConnectNGame(3, 3, 3);
ConnectNGame game2 = new ConnectNGame(6, 7, 4);
ConnectNGame game3 = new ConnectNGame(15, 15, 5);
ConnectNGame game4 = new ConnectNGame(5, 5, 4);
ConnectNGame game5 = new ConnectNGame(10, 10, 5);
